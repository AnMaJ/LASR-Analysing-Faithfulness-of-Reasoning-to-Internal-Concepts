...
# ==============================================================================
# Per-Feature Index Plots (x = feature index, y = mean / variance)
# ==============================================================================

def plot_feature_index_scatter(
    means: np.ndarray,
    variances: np.ndarray,
    layer_idx: int,
    agg_method: str,
    ...
    """
    Plot per-feature mean and variance as a function of feature index.
    Top panel:    mean activation per feature (across 100 samples)
    Bottom panel: variance of activation per feature (across 100 samples)
    ...
    # --- Mean scatter ---
    ax = axes[0]
    ax.scatter(feature_indices, means, s=0.15, alpha=0.5,
               color=config.COLOR_MEAN, rasterized=True)
    # Running average to show trend (window = 200 features)
    window = min(200, num_features // 10)
    if window > 1:
        running_mean = np.convolve(means, np.ones(window) / window, mode="same")
        ax.plot(feature_indices, running_mean, color="#0D47A1",
                linewidth=0.8, alpha=0.7, label=f"Running avg (w={window})")
        ax.legend(fontsize=8, loc="upper right")
    ax.set_ylabel("Mean Activation", fontsize=11)
    ax.set_title(f"Per-Feature Mean Activation (across samples)", fontsize=12, fontweight="bold")
    ax.grid(True, alpha=0.15)
    # Annotate stats
    nonzero = means[means > 0]
    ax.text(0.99, 0.92,
            f"Global mean: {means.mean():.4f}\n"
            f"Max: {means.max():.4f}\n"
            f"Non-zero: {len(nonzero):,}/{num_features:,}",
            transform=ax.transAxes, ha="right", va="top", fontsize=8,
            bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.85))

    # --- Variance scatter ---
    ax = axes[1]
    ax.scatter(feature_indices, variances, s=0.15, alpha=0.5,
               color=config.COLOR_VARIANCE, rasterized=True)
    if window > 1:
        running_var = np.convolve(variances, np.ones(window) / window, mode="same")
        ax.plot(feature_indices, running_var, color="#BF360C",
                linewidth=0.8, alpha=0.7, label=f"Running avg (w={window})")
        ax.legend(fontsize=8, loc="upper right")
    ax.set_ylabel("Variance", fontsize=11)
    ax.set_xlabel("Feature Index", fontsize=11)
    ax.set_title(f"Per-Feature Variance of Activation (across samples)", fontsize=12, fontweight="bold")
    ax.grid(True, alpha=0.15)
    nonzero_v = variances[variances > 0]
    ax.text(0.99, 0.92,
            f"Global var: {variances.mean():.4f}\n"
            f"Max: {variances.max():.4f}\n"
            f"Non-zero: {len(nonzero_v):,}/{num_features:,}",
            transform=ax.transAxes, ha="right", va="top", fontsize=8,
            bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.85))

    fig.suptitle(f"Layer {layer_idx} — {agg_label}\n"
                 f"Feature-level activation statistics ({num_features:,} features)",
                 fontsize=14, fontweight="bold", y=1.02)
    fig.tight_layout()

    filename = f"layer_{layer_idx}_{agg_method}_feature_scatter.png"
    filepath = os.path.join(save_dir, filename)
    fig.savefig(filepath, dpi=config.FIG_DPI, bbox_inches="tight",
                facecolor="white", edgecolor="none")
    plt.close(fig)
...
def plot_feature_index_grid(
    ...
    """
    Create all-layers grids where each subplot is feature-index vs mean (or variance).
    Returns (mean_grid_path, variance_grid_path).
    """
    ...
    for idx, layer in enumerate(layers):
        ...
        ax.scatter(np.arange(len(means)), means, s=0.05, alpha=0.3,
                   color=config.COLOR_MEAN, rasterized=True)
    ...
    fig_mean.suptitle(f"Per-Feature MEAN vs Feature Index — {agg_label} — All Layers",
                      fontsize=13, fontweight="bold", y=1.02)
    fig_mean.tight_layout()
    mean_path = os.path.join(save_dir, f"grid_all_layers_{agg_method}_feature_mean.png")
    ...
    for idx, layer in enumerate(layers):
        ...
        ax.scatter(np.arange(len(variances)), variances, s=0.05, alpha=0.3,
                   color=config.COLOR_VARIANCE, rasterized=True)
        ax.set_title(f"L{layer}", fontsize=8, fontweight="bold")
        ax.tick_params(labelsize=5)
        ax.yaxis.set_major_locator(MaxNLocator(nbins=3))
        if col == 0:
            ax.set_ylabel("Var", fontsize=6)

    for idx in range(num_layers, nrows * ncols):
        row, col = divmod(idx, ncols)
        axes_var[row, col].set_visible(False)

    fig_var.suptitle(f"Per-Feature VARIANCE vs Feature Index — {agg_label} — All Layers",
                     fontsize=13, fontweight="bold", y=1.02)
    fig_var.tight_layout()
    var_path = os.path.join(save_dir, f"grid_all_layers_{agg_method}_feature_variance.png")
...
# ==============================================================================
# All-Layers Grid Plot (Histograms)
# ==============================================================================
...
def run_plotting(config: PlotConfig, logger: logging.Logger, demo: bool = False):
    ...
    for agg_method, agg_label in config.AGG_METHODS.items():
        ...
        for layer_idx in layers:
            ...
            # Individual layer feature-index scatter
            scatter_path = plot_feature_index_scatter(
                means, variances, layer_idx, agg_method, agg_label,
                config, agg_dir)
            all_plot_paths.append(scatter_path)
        ...
        # Grid plots for all layers (histograms)
        logger.info(f"  Generating all-layers histogram grids for {agg_method}...")
        ...
        logger.info(f"  Histogram grid plots saved: {mean_grid}, {var_grid}")

        # Grid plots for all layers (feature-index scatter)
        logger.info(f"  Generating all-layers feature-index grids for {agg_method}...")
        feat_mean_grid, feat_var_grid = plot_feature_index_grid(
            all_means_dict, all_vars_dict, agg_method, agg_label,
            config, config.PLOT_DIR)
        all_plot_paths.extend([feat_mean_grid, feat_var_grid])
        logger.info(f"  Feature-index grid plots saved: {feat_mean_grid}, {feat_var_grid}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--plot-dir", type=str, default="/vol/bitbucket/m24/Concept_Neuron_Localisation_my_idea/Investigating_concept_neurons/LASR-Analysing-Faithfulness-of-Reasoning-to-Internal-Concepts/outputs/plots_scatter",
