"""Generate transcoder activations for the natural counterfactuals dataset and save results.

Loads question pairs from data/natural_counterfactuals/combined_dataset.parquet,
applies a chat template to both original and counterfactual prompts, generates
text + transcoder activations, and saves everything to an activations folder.

Each row produces two samples (original and counterfactual), linked by a shared
pair index.
...
Usage:
    python -m src.utils.store_activations_counterfactuals [--batch_size 8] [--max_new_tokens 1024] [--output_dir activations]
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate and save transcoder activations for natural counterfactuals")
    ...
    parser.add_argument("--parquet_path", type=str, default="data/natural_counterfactuals/combined_dataset.parquet",
                        help="Path to combined counterfactual parquet file")
    parser.add_argument("--max_samples", type=int, default=None,
                        help="Max number of question pairs to process (default: all)")
...
def load_counterfactuals(parquet_path: str, model: GemmaModel, max_samples: int | None = None):
    """Load counterfactual pairs from parquet, wrap in chat template, return prompts + metadata.

    Returns flat lists where original and counterfactual prompts are interleaved:
    [orig_0, cf_0, orig_1, cf_1, ...]. Metadata tracks which pair each sample belongs to.
    """
    ...
    all_prompts = []
    pair_indices = []
    sample_types = []  # "original" or "counterfactual"
    original_question_indices = []
    counterfactual_question_indices = []
    original_datasets = []

    for pair_idx, (_, row) in enumerate(df.iterrows()):
        for side, prompt_col, q_idx_col in [
            ("original", "original_question_prompt", "original_question_idx"),
            ("counterfactual", "counterfactual_question_prompt", "counterfactual_question_idx"),
        ]:
            ...
            sample_types.append(side)
            original_question_indices.append(int(row["original_question_idx"]))
            counterfactual_question_indices.append(int(row["counterfactual_question_idx"]))
            original_datasets.append(row["original_dataset"])

    print(f"Total samples: {len(all_prompts)} ({len(df)} pairs)")
    return all_prompts, pair_indices, sample_types, original_question_indices, counterfactual_question_indices, original_datasets
...
def main():
    ...
    # --- Load dataset ---
    (all_prompts, pair_indices, sample_types,
     original_question_indices, counterfactual_question_indices,
     original_datasets) = load_counterfactuals(args.parquet_path, model, args.max_samples)
    ...
    filename = f"counterfactuals-transcoder-l{args.transcoder_layer}-{args.transcoder_width}{affine_tag}"
    ...
    torch.save({
        ...
        "dataset_info": {
            ...
            "original_question_indices": original_question_indices,
            "counterfactual_question_indices": counterfactual_question_indices,
    ...
    print(f"  Pairs: {max(pair_indices) + 1}")
